...
interface PropertySubmenusProps {
  project: ProjectData;
  onUpdateStatus?: (status: string) => void;
  onUpdatePriority?: (priority: string) => void;
  onUpdateHealth?: (health: string) => void;
...
const ProjectPropertySubmenus: React.FC<PropertySubmenusProps> = ({
  project,
  onUpdateStatus,
  onUpdatePriority,
  onUpdateHealth,
    ...
    <>
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-48 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdateStatus?.("PAUSED")} className="cursor-pointer">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-56 max-h-60 overflow-y-auto bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          {teamUsers.map((u) => (
            <ContextMenuItem key={u.id} onClick={() => onUpdateLead?.(u.name || u.email)} className="cursor-pointer flex items-center gap-2">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-44 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdateHealth?.("At risk")} className="text-amber-500 font-medium cursor-pointer">
            <AlertTriangle className="mr-2 h-3.5 w-3.5 text-amber-500" />
            <span>At risk</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdateHealth?.("Off track")} className="text-destructive font-medium cursor-pointer">
            <AlertCircle className="mr-2 h-3.5 w-3.5 text-destructive" />
            <span>Off track</span>
          </ContextMenuItem>
        </ContextMenuSubContent>
...
const ProjectActions: React.FC<ProjectActionsProps> = ({ project, onDelete }) => {
  ...
  return (
    <>
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-52 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          >
            ...
            <ContextMenuShortcut className="font-mono text-[9px]">
              {project.obsidianRef || "ID"}
            </ContextMenuShortcut>
          </ContextMenuItem>
          <ContextMenuItem
            onClick={() =>
      ...
      {/* Favorite */}
      <ContextMenuItem
        onClick={() => toast.success(`Project "${project.name}" added to favorites`)}
        className="cursor-pointer"
      >
