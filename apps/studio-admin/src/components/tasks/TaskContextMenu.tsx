...
interface TaskContextMenuProps {
  ...
  onUpdateScope?: (scope: TaskScope) => void;
  onDelete?: () => void;
  children: React.ReactNode;
}

interface TaskPropertySubmenusProps {
  task: Task;
  onUpdateStatus?: (status: string) => void;
  onUpdatePriority?: (priority: string) => void;
  onUpdateAssignee?: (assigneeId?: string) => void;
...
const TaskPropertySubmenus: React.FC<TaskPropertySubmenusProps> = ({
  task,
  onUpdateStatus,
  onUpdatePriority,
  onUpdateAssignee,
    ...
    <>
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-48 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdateStatus?.("TODO")} className="cursor-pointer">
            <CircleDot className="mr-2 h-3.5 w-3.5 text-blue-400" />
            <span>To Do</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdateStatus?.("IN_PROGRESS")} className="cursor-pointer">
            <Clock className="mr-2 h-3.5 w-3.5 text-amber-500" />
            <span>In Progress</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdateStatus?.("WAITING_ON_CLIENT")} className="cursor-pointer">
            <Clock className="mr-2 h-3.5 w-3.5 text-purple-400" />
            <span>Waiting on Client</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdateStatus?.("RESOLVED")} className="cursor-pointer font-medium text-primary">
            <CheckCircle2 className="mr-2 h-3.5 w-3.5 text-primary" />
            <span>Resolved</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdateStatus?.("CLOSED")} className="cursor-pointer text-muted-foreground">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-44 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdatePriority?.("HIGH")} className="text-amber-500 font-semibold cursor-pointer">
            <ArrowUp className="mr-2 h-3.5 w-3.5 text-amber-500" />
            <span>High</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdatePriority?.("NORMAL")} className="cursor-pointer">
            <Minus className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
            <span>Normal</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => onUpdatePriority?.("LOW")} className="cursor-pointer">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-56 max-h-60 overflow-y-auto bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          {teamUsers.map((u) => (
            <ContextMenuItem key={u.id} onClick={() => onUpdateAssignee?.(u.name || u.email)} className="cursor-pointer flex items-center gap-2">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-48 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdateDueDate?.(undefined)} className="text-muted-foreground cursor-pointer">
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-48 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => onUpdateScope?.("TENANT_TO_PLATFORM")} className="cursor-pointer">
    ...
    <>
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent className="w-52 bg-popover/95 backdrop-blur-xl border-border shadow-2xl z-[10000] rounded-xl p-1">
          ...
          <ContextMenuItem onClick={() => handleCopy(task.title, "Issue Title")} className="cursor-pointer">
            <span>Copy title</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => handleCopy(gitBranchName, "Git Branch")} className="cursor-pointer">
            <GitBranch className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
            <span>Copy branch name</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => handleCopy(task.id, "Issue ID")} className="cursor-pointer">
      ...
      {/* Favorite */}
      <ContextMenuItem onClick={() => toast.success(`Task "${task.title}" added to favorites`)} className="cursor-pointer">
      ...
      {/* Open in new tab */}
      <ContextMenuItem onClick={() => window.open(`/tasks?issue=${task.id}`, "_blank")} className="cursor-pointer">
        <ExternalLink className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
