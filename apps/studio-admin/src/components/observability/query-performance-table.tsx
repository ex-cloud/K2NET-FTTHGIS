...
function buildColumns(
  ...
  return [
    ...
    columnHelper.display({
      ...
      header: "Application",
      cell: () => (
        <span className="block text-xs font-mono text-muted-foreground text-right select-all">-</span>
      ),
    }),
  ];
}

function TableHeaderCell({ header }: { header: Header<SlowQuery, unknown> }) {
  ...
  return (
    <div className={`min-w-0 px-4 py-2.5 flex items-center ${isRightAligned ? "justify-end text-right" : "justify-start text-left"}`}>
      {canSort && header.column.id !== "application" ? (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className="flex items-center gap-1 hover:text-foreground transition-colors outline-hidden select-none py-1 px-1.5 -mx-1.5 rounded hover:bg-muted/40 font-medium cursor-pointer"
            >
...
export function QueryPerformanceTable({
  ...
  const columns = React.useMemo(
    () => buildColumns(copiedIdx, onCopy, onSelectQuery),
