...
import { Radio, Network, AlertTriangle } from "lucide-react";
import { oltDevicesMock } from "@/lib/mock-data/observability-mock";

export default function OltPollerPage() {
  const online = oltDevicesMock.filter(d => d.snmpStatus === "OK").length;
  return (
    <PageLayout variant="dashboard" spaceY="space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-5">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">SNMP device polling, optical attenuation readings, and OLT connectivity status.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { label: "Active OLT Connections", value: `${online} / ${oltDevicesMock.length}`, sub: "SNMP reachable devices" },
          { label: "SNMP Ping Success", value: "96%", sub: "Last polling cycle: 3 min ago" },
          { label: "SSH Session Failures", value: "0", sub: "No SSH errors in last 24h" },
          { label: "Total Polled ODP", value: "236", sub: "Across all active OLTs" },
        ].map((c) => (
      ...
      <Card>
        <CardHeader className="border-b border-border pb-4">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Physical OLT devices monitored by ftth-olt-gateway and ftth-poller.</p>
        </CardHeader>
        <CardContent className="p-0">
          <div className="grid grid-cols-[1fr_110px_60px_80px_70px_90px] px-5 py-2 border-b border-border bg-muted/30 gap-2">
          ...
          <div className="divide-y divide-border">
            {oltDevicesMock.map((d) => (
              <div key={d.hostname} className="grid grid-cols-[1fr_110px_60px_80px_70px_90px] px-5 py-3.5 hover:bg-muted/20 transition-colors items-center gap-2">
                <div>
