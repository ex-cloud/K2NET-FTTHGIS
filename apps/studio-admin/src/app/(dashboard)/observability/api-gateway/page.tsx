...
function ApiGatewayKpis({
  ...
  const cards = [
    ...
    {
      label: "Active Routes",
      value: loadingRoutes ? "…" : String(routesCount),
...
function ApiGatewayTrafficChart({
  isKongUp,
  trafficHistory,
}: {
  isKongUp: boolean;
  trafficHistory: Array<{ hour: string; api: number; gateways: number }>;
}) {
  return (
    <Card>
...
function ApiGatewayRoutesTable({
  routes,
  loadingRoutes,
  routesError,
}: {
  routes: Array<{ routeId?: string; route: string; upstream: string; methods: string; plugins: string[]; status: "UP" | "DOWN" | "UNKNOWN" }>;
  loadingRoutes: boolean;
...
export default function ApiGatewayPage() {
  ...
  const hasError = Boolean(routesError || trafficError);
  ...
  return (
    <PageLayout variant="workspace" spaceY="space-y-6">
      <ApiGatewayHeader
        hasError={hasError}
        errorMsg={routesError ?? trafficError}
        isKongUp={isKongUp}
      ...
      <ApiGatewayKpis
        ...
        workerMemoryMiB={workerMemoryMiB}
        workerCount={workerCount}
      />

      <ApiGatewayTrafficChart
        isKongUp={isKongUp}
        trafficHistory={trafficHistory}
      />

      <ApiGatewayRoutesTable
        routes={routes}
        loadingRoutes={loadingRoutes}
