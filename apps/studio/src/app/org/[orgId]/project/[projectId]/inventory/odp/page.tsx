...
export default function OdpListPage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await networkApi.deleteAsset("ODP", odpToDelete.id, deleteReason, session.accessToken as string, projectId as string);
  ...
  const handleBulkDelete = async (selected: ODP[]) => {
    ...
    try {
      await networkApi.batchDelete("ODP", selected.map(item => item.id), reason, session?.accessToken as string, projectId as string);
          ...
          return (
            <div className="flex flex-col gap-1.5 min-w-[120px]">
              <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tight">
                ...
                <span className={pct > 80 ? "text-amber-500 font-black" : "text-primary"}>{pct}%</span>
              </div>
              <div className="h-1.5 w-full bg-zinc-800/50 rounded-full overflow-hidden border border-border shadow-inner">
                <div
