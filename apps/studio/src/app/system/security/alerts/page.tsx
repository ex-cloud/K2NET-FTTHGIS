    ...
    <div className="flex-1 flex flex-col pt-16 px-8 bg-[#0c0c0c] h-full overflow-y-auto custom-scrollbar select-none text-zinc-100">
      <div className="w-full max-w-7xl mx-auto space-y-8 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          <Card className="bg-zinc-900/35 border-zinc-800/80 shadow-md backdrop-blur-sm">
          ...
          <Card className="bg-zinc-900/35 border-zinc-800/80 shadow-md backdrop-blur-sm">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <p className="text-xs text-zinc-400 font-medium">Critical Alerts</p>
          ...
          <Card className="bg-zinc-900/35 border-zinc-800/80 shadow-md backdrop-blur-sm">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <p className="text-xs text-zinc-400 font-medium">Warning Anomalies</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <Card className="bg-zinc-900/30 border-zinc-800/80 shadow-xl backdrop-blur-sm">
              ...
              <CardContent className="pt-5 space-y-4">
                <form onSubmit={handleBlockIp} className="space-y-3">
                  <div className="space-y-1">
                    <Label htmlFor="block_ip" className="text-zinc-400 text-[10px] uppercase font-bold tracking-wider">IP Address / CIDR Range</Label>
                    <Input
                      id="block_ip"
                      placeholder="e.g. 103.111.12.5 or 192.168.1.0/24"
                      value={ipInput}
                      onChange={(e) => setIpInput(e.target.value)}
                      className="bg-zinc-950/60 border-zinc-850 text-zinc-200 text-xs h-9"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="block_reason" className="text-zinc-400 text-[10px] uppercase font-bold tracking-wider">Block Reason</Label>
                    <Input
                      id="block_reason"
                  ...
                  <Button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs h-9 font-medium transition-all shadow-md gap-2"
                  >
                ...
                <div className="border-t border-zinc-850 pt-4">
                  <Label className="text-zinc-400 text-[10px] uppercase font-bold tracking-wider block mb-2">Active Rules ({blockedIps.length})</Label>
                  ...
                  {loadingBlockedIps ? (
                    ...
                    <div className="text-center p-4 border border-dashed border-zinc-850 rounded-lg bg-zinc-950/20 text-zinc-500 text-xs">
                      No IP blocking rules configured.
                    </div>
                  ) : (
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar border border-zinc-850 rounded-lg bg-zinc-950/40">
                      <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody>
                          {blockedIps.map((rule) => (
                            <tr key={rule.id} className="border-b border-zinc-850/40 text-zinc-300 hover:bg-zinc-900/10">
                              <td className="p-2 font-mono text-[10px] text-zinc-200">{rule.ipAddressOrCidr}</td>
                              <td className="p-2 text-[10px] text-zinc-400 max-w-[120px] truncate" title={rule.reason}>{rule.reason}</td>
            ...
            <Card className="bg-zinc-900/30 border-zinc-800/80 shadow-xl backdrop-blur-sm">
              ...
              <CardContent className="pt-5 p-3">
                {loadingAlerts ? (
                  ...
                  <div className="text-center p-8 border border-dashed border-zinc-850 rounded-xl bg-zinc-950/20 text-zinc-500 text-xs">
                        ...
                        <div
                          ...
                          className={`p-3.5 border rounded-xl cursor-pointer transition-all ${
                            selectedAlert?.id === alert.id
                              ...
                              : "border-zinc-850 bg-zinc-950/45 hover:border-zinc-800 hover:bg-zinc-900/10"
                          }`}
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="space-y-1">
                              ...
                              <h4 className="text-xs font-semibold text-zinc-200 mt-1">{alert.username}</h4>
                              <p className="text-[10px] text-zinc-400 font-mono">{alert.ipAddress} ({alert.location || "Unknown"})</p>
                            </div>
                          ...
                          {selectedAlert?.id === alert.id && (
                            <div className="mt-3 pt-3 border-t border-zinc-850/60 text-[10px] text-zinc-400 leading-relaxed font-sans space-y-2">
                              <p className="text-zinc-300 bg-zinc-950/80 p-2 rounded-lg border border-zinc-850/40">{alert.details}</p>
                              {alert.os && (
                                <div className="grid grid-cols-2 gap-2 text-[9px] font-mono text-zinc-500 pt-1">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* Attack Map Card */}
            <Card className="bg-zinc-900/30 border-zinc-800/80 shadow-xl backdrop-blur-sm overflow-hidden">
              ...
              <div className="relative h-[250px] w-full bg-zinc-950">
                <Map
                  {...viewState}
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Mini Tooltip */}
                          <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1 bg-zinc-950 border border-zinc-850 px-2 py-1 rounded text-[9px] font-mono text-zinc-300 opacity-0 group-hover:opacity-100 pointer-events-none transition-all whitespace-nowrap z-50">
                ...
                {selectedAlert && (
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 rounded-md bg-zinc-950/80 border border-zinc-800 text-zinc-400 hover:text-zinc-200 z-10 transition-colors"
                  >
              ...
              <CardContent className="pt-5 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <Label htmlFor="sim_user" className="text-zinc-500 text-[9px]">User ID</Label>
                      <Input
                        id="sim_user"
                        value={simUserId}
                        onChange={(e) => setSimUserId(e.target.value)}
                        className="bg-zinc-950/60 border-zinc-850 text-zinc-300 text-[10px] h-7"
                      />
                    </div>
                    <div className="space-y-1">
                      <Label htmlFor="sim_username" className="text-zinc-500 text-[9px]">Username</Label>
                      <Input
                        id="sim_username"
                        value={simUsername}
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="sim_ip" className="text-zinc-500 text-[9px]">Simulated Remote IP</Label>
                    <Input
                      id="sim_ip"
                      value={simIp}
                ...
                <div className="border-t border-zinc-850/60 pt-4 space-y-3">
                  {/* Brute Force Simulation */}
                  <h4 className="text-xs font-semibold text-rose-400 uppercase tracking-wider font-mono">2. Brute Force Login</h4>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <Label htmlFor="sim_fail_ip" className="text-zinc-500 text-[9px]">Attacker IP</Label>
                      <Input
                        id="sim_fail_ip"
                        value={simFailIp}
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="sim_fail_count" className="text-zinc-500 text-[9px]">Fail Count</Label>
                      <Input
                        id="sim_fail_count"
                        type="number"
                        min={1}
