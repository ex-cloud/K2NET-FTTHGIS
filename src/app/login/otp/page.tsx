...
export default function OtpPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-zinc-950">
        ...
        <div className="relative z-10 w-full max-w-md mx-4">
          <div className="backdrop-blur-xl bg-zinc-900/80 border border-zinc-800/80 rounded-2xl shadow-2xl p-8 animate-pulse">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-zinc-950">
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        <div className="backdrop-blur-xl bg-zinc-900/80 border border-zinc-800/80 rounded-2xl shadow-2xl p-8">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight flex items-center justify-center gap-2">
              Verifikasi 2 Langkah <ShieldCheck className="h-5 w-5 text-emerald-500" />
            </h1>
          ...
          {!sessionReady ? (
            <div className="rounded-lg bg-zinc-800/40 border border-zinc-700/20 p-3 mb-6 animate-pulse">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="h-6 w-20 bg-zinc-700/50 rounded-md" />
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between gap-3 rounded-lg bg-zinc-800/40 border border-zinc-700/30 p-3 text-xs text-zinc-300 mb-6 transition-all duration-500 animate-in fade-in">
              <div className="flex items-center gap-3">
                <Laptop className="h-5 w-5 text-primary shrink-0" />
                <div>
                  <p className="font-semibold text-zinc-200">Perangkat Saat Ini</p>
                  <p className="text-[10px] text-zinc-500">{browser} ({os})</p>
                </div>
              </div>
              <button
                type="button"
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-semibold border rounded-lg transition-all shrink-0 ${
                  isTrustingDevice
                    ? "text-zinc-400 border-zinc-700 bg-zinc-800/30 cursor-not-allowed"
                    : "text-emerald-400 border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 hover:border-emerald-500/50 hover:text-emerald-300"
                }`}
              >
                {isTrustingDevice ? (
                  <>
          ...
          {!otpSent ? (
            <form onSubmit={handleSendOtp} className="space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-zinc-400">Pilih Metode Pengiriman OTP</label>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-3.5 rounded-xl border text-center transition-all ${
                      method === "email"
                        ...
                        : "border-zinc-800 bg-zinc-900/40 hover:bg-zinc-800/40 text-zinc-400"
                    }`}
                  >
              ...
              {method === "email" && (
                <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-3.5 text-center">
                  <p className="text-[10px] text-zinc-500">KODE AKAN DIKIRIM KE ALAMAT EMAIL TERDAFTAR</p>
                  <p className="text-xs font-semibold text-zinc-300 mt-1">{session?.user?.email || "Email akun"}</p>
                </div>
              )}
              ...
              {method === "whatsapp" && (
                <div className="space-y-2 animate-fadeIn">
                  ...
                  <Input
                    ...
                    className="h-11 bg-zinc-950 border-zinc-800 text-sm focus:border-primary/50 text-white"
                  />
                  <p className="text-[10px] text-zinc-500">Masukkan nomor yang aktif untuk menerima OTP.</p>
                </div>
              )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2.5">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-semibold text-zinc-400">Masukkan 6 Digit OTP</label>
                  {countdown > 0 ? (
                    <span className="text-[10px] text-zinc-500">Kirim ulang dalam {countdown}s</span>
                  ) : (
                    <button 
                ...
                <div className="flex justify-between gap-2.5">
                  {otp.map((digit, idx) => (
                    <input
                      ...
                      className="w-12 h-12 bg-zinc-950 border border-zinc-850 focus:border-primary focus:ring-1 focus:ring-primary rounded-lg text-center text-xl font-bold text-white outline-hidden transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full h-11 border-zinc-800 bg-zinc-900/30 text-zinc-400 hover:bg-zinc-800/40 text-xs"
                >
          ...
          {/* Footer Back Link */}
          <div className="mt-8 pt-5 border-t border-zinc-850 text-center">
            <button
